...
*First*, if a party can choose its own index and the implementation rejects only the literal integer `0`, an attacker submitting $i = q$ (or any $k \cdot q$) passes the check while `evaluatePolynomial(q) ≡ evaluatePolynomial(0) = f(0) = secret`, handing it the secret directly. The same shape appears when indices are typed as small unsigned integers (e.g., `u16`) and the zero check is written against the type's default rather than the field zero.
...
**Security implication.** The non-zero failure hands the attacker the shared secret as their "share": the check passes, honest parties send the share unmodified, and the attacker holds the full private key after a normal sharing ceremony, with no further interaction required. The duplicate failure splits into two outcomes. In availability terms, reconstruction crashes with a nil-pointer dereference (Go's `ModInverse` returns `nil` for a non-invertible input) or throws an unrecoverable error, DoS-ing the signing ceremony. In integrity terms, some implementations silently skip the offending term or substitute a default, producing an incorrect reconstruction the caller accepts as valid.
