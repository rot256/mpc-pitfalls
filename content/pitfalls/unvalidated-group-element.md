...
**Security implication.** Concretely in GG18 MtA, as [analyzed by Hexens](https://hexens.io/blog/mpc-attacks-p1), an adversary can set $h_2 = 1$ so $z = h_1^s \bmod \tilde N$ leaks $h_1^s$. When $\tilde N$ is generated without enforcing safe primes (see [non-safe-prime modulus](non-safe-prime-modulus.md)), the attacker can choose $\tilde N$ as a product of small prime factors so that $\phi(\tilde N)$ is smooth, and combining Pohlig-Hellman on each factor with CRT reconstructs the full share (see [Valenta et al., eprint 2016/995](https://eprint.iacr.org/2016/995.pdf) for the canonical analysis). Alternatively, an attacker can choose $\tilde N$ large enough that recovering $s$ reduces to computing integer logarithms, which is trivial with standard algorithms.
